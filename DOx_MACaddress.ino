...
int GV_FIND=0;
float GV_TOOHIGHVALUE = 10.00;
float GV_TOOLOWVALUE = 6.00;
...
void loop() {
  if (GV_FIND){                                                         // GV_FIND is set to true if web page ipaddress/findon is called.  Nothing is supposed to function while this is happening.
    ...
    while( GV_FIND == 1 ){                                              // GV_FIND is set to false if web page ipaddress/findoff is called.
      fill_solid(leds, NUM_LEDS, CRGB(255,  255,  255));   //white
      delay(40);
      FastLED.show();
      fill_solid(leds, NUM_LEDS, CRGB(0,  0,  0));   //white
      delay(40);
      FastLED.show();
    }
    LED_Clear();
    //leds[6] = CRGB::Black;    // *** for some reason led[6] stays on after calling LED_Clear above (doesn't happend anywhere else it's called), so make sure it goes off.
    FastLED.setBrightness(5);
  }
  ...
  if (((millis() - DOxReadingMillis) > 2000) && !GV_WEB_REQUEST_IN_PROGRESS){
    ...
    LCD_DISPLAY("  ", GV_DOX_DATA.length(), 1, NoClearLCD, NoSerial);
...
void LCDshowHeartBeat() {
  if ((millis() - HeartBeatMillis) > 1000) {
    if ( (GV_DOX_DATA.toFloat() >= GV_TOOHIGHVALUE) || (GV_DOX_DATA.toFloat() <= GV_TOOLOWVALUE) ){
        for(int v=0; v<15; v++){
          fill_solid(leds, NUM_LEDS, CRGB(255,  255,    0));          //yellow
          FastLED.show();
          delay(30);
          LED_Clear();
          delay(30);
...
void SendCommandAndSetDOxVariables(String command) {
  ...
  if (Ccommand[0] != 'r') LED_DOreturn_code_Fade(code);
